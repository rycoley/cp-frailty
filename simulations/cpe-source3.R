...
#For estimation with noninformative priors 
#Accompanies cpe-cpt3.R and cpe-to-run3.R
...
#comparable to cpe-source.R. see that code for annotations
...
get.lambda<-function(delta, Zs, bet, Y){ #uniform prior
	return(rgamma(1,shape=sum(delta)+1, rate=sum(Zs*exp(X*bet)*Y)))}
...
slice.eta<-function(eta0, rho, Nhat, Zhat, eta.s, eta.r){ 
	...
	while(L>1e-20 & lik.eta(etah=L, rho=rho, Ns=Nhat, Zs=Zhat) > z  & J>0){L<- max(L-w,1e-20)
		J<- J-1}
	while(lik.eta(etah=R, rho=rho, Ns=Nhat, Zs=Zhat) > z & K>0){R<- R+w
		K<- K-1}
...
lik.Ni<-function(Ni,rho,eta,lexbi,Yi){ 
	nu<-rho*eta
...
get.yrep.surv<-function(bet, lam, rhohat, etahat, ER, pdf.true, Tf, Tf.mo){
	...
	Y.rep<-rep(500,n)
	for(i in 1:n){if(Z.rep[i]>0){Y.rep[i]<-rexp(1, rate=lam*Z.rep[i]*exp(X[i]*bet))}
		else{Y.rep[i]<-5000}}
