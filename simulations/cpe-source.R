...
#Accompanies cpe-cpt.R and cpe-to-run.R for correct priors,
#cpe-cpt1.R, cpe-to-run1.R, cpe-cpt2.R for incorrect but informative priors
...
n2<-1500 
n<-n1+n2
X<-c(rep(0,n1), rep(1,n2)) 	#treatment indicator

####### FUNCTIONS, constant across simualations
#defined with fixed values (above) but not simulated data (from cpe-cpt-to-run.R)
...
slice.beta<-function(beta0, lam, Zhat, delta, Y){ 
	...
	z<- lik.beta(bet=beta0, lam=lam, Zs=Zhat, delta=delta, Y=Y) - rexp(1,1)#want to make a slice with all betas that have a log-likelihood above this
...
#sample constant baseline hazard (lambda) 
...
#log prior density for number of exposure processes (rho)
#prior(PNR)~beta(A,B)
...
#log-posterior likelihood for mean number of exposure processes (rho)
...
slice.rho<-function(rho0, eta, Nhat, Zhat, rho.a, rho.b){ 
...
#log-posterior likelihood for eta, the shape parameter of the gamma random variable for amount of risk associated with each exposure process
